...
namespace EQPAct
{
    public class clsEQPEventPPIDDelete : clsEQPEvent, IEQPEvent
    {
        #region Variables
        #endregion
        ...
        #region Constructors
        public clsEQPEventPPIDDelete(string strUnitID)
        {
            m_strUnitID = strUnitID;
            strEventName = "actPPIDDelete";
        }
        #endregion
        ...
        public void funProcessEQPEvent(string[] parameters)
        {
            ...
            try
            {
                ...
                switch (dintPPIDType)
                {
                    case 1:         //EQP PPID
                        {
                            pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S7F107PPIDCreatDeleteAndPPBodyChangedReport, "2", "1", "", dstrEQPPPID);
                            pInfo.Unit(0).SubUnit(0).RemoveEQPPPID(dstrEQPPPID);
                            ...
                            {
                                pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S7F107PPIDCreatDeleteAndPPBodyChangedReport, "2", "2", arrCon[dintLoop].ToString(), dstrEQPPPID);

                                pInfo.Unit(0).SubUnit(0).RemoveHOSTPPID(arrCon[dintLoop].ToString());
                                dstrSQL = string.Format("Delete FROM `tbHOSTPPID` Where HOSTPPID='{0}';", arrCon[dintLoop].ToString());
