...
namespace EQPAct
{
    public class clsEQPEventCaseOut : clsEQPEvent, IEQPEvent
    {
        #region Variables
        #endregion
        ...
        #region Constructors
        public clsEQPEventCaseOut(string strUnitID)
        {
            m_strUnitID = strUnitID;
            strEventName = "actCaseOut";
        }
        #endregion
        ...
        public void funProcessEQPEvent(string[] parameters)
        {
            ...
            try
            {
                ...
                {
                    dstrWordAddress = "W2290";
                    dstrWordAddress = CommonAct.FunTypeConversion.funAddressAdd(dstrWordAddress, 32 * (dintSubUnitID - 1));
                    dstrCaseID = m_pEqpAct.funWordRead(dstrWordAddress, 8, EnuEQP.PLCRWType.ASCII_Data);
                }
                pInfo.subLog_Set(InfoAct.clsInfo.LogType.CIM , string.Format ( "CaseOut ==> Case ID : {0}", dstrCaseID));
                ...
                if (dintPortID == 2)
                {
                    ...
                    int dintCount = Convert.ToInt32( m_pEqpAct.funWordRead("W2238", 1, EnuEQP.PLCRWType.Int_Data));
                    //this.pInfo.subPLCCommand_Set(InfoAct.clsInfo.PLCCommand.FilmJobCommand, 1004, dintPortID);     //Lot Start 지시
                    pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S6F11RelatedJobProcess, 1004, 2, 1, 2, pInfo.Port(2).CSTID, dintCount);

                }
                //this.pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S6F11MaterialLoadEvent, 1135, "Out", dintPortID, dstrCaseID, dintUnitID, dintSubUnitID);

                if (dintPortID == 1 || dintPortID == 2|| dintPortID == 5)
                {
                    this.pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S6F11RelatedPortEvent, 35, "OUT", dintPortID, dstrCaseID, dintUnitID, dintSubUnitID);
                    if (dintPortID == 1)
                    {
                        pInfo.subPLCCommand_Set(clsInfo.PLCCommand.FI01_Check, "0");                        
                    }
                ...
                else if (dintPortID == 4 || dintPortID == 6)
                {

                    this.pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S6F11RelatedPortEvent, 34, "OUT", dintPortID, dstrCaseID, dintUnitID, dintSubUnitID);
                    this.pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S6F11RelatedPortEvent, 35, "OUT", dintPortID, dstrCaseID, dintUnitID, dintSubUnitID);
                ...
                {
                    //this.pInfo.subSendSF_Set(InfoAct.clsInfo.SFName.S6F11CSTMovingEvent, 45, "OUT", dintPortID, dstrCaseID, dintUnitID, dintSubUnitID);
                }
