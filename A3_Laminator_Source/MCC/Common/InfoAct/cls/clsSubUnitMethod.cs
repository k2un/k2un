...
namespace InfoAct
{
    public class clsSubUnitMethod
    {
        ...
        //MCC Log Data 개체등록 및 가져오기
        #region "MCC Log"

        public bool AddMCC(int intIndex)
        {
            clsMCC dclsMCC;

            try
            {
                int dintMCCIndex = intIndex;

                if (pHashtableMCC.Contains(dintMCCIndex))
                {
                    return false;
                }

                dclsMCC = new clsMCC(dintMCCIndex);
                pHashtableMCC.Add(dintMCCIndex, dclsMCC);
        ...
        public clsMCC MCC(int intMCCIndex)
        {
            try
            {
                int dintMCC = intMCCIndex;

                if (pHashtableMCC.Contains(dintMCC))
                {
                    return (clsMCC)pHashtableMCC[dintMCC];
                }
                else
                {
                    return null;
        ...
        public ICollection MCC()
        {
            try
            {
                return pHashtableMCC.Keys;
            }
            catch
            {
                return null;
        ...
        public bool RemoveMCC(int intMCCIndex)
        {
            try
            {
                int dintMCCIndex = intMCCIndex;

                if (pHashtableMCC.Contains(dintMCCIndex))
                {
                    pHashtableMCC.Remove(dintMCCIndex);     //HashTable에서 해당 LOT을 제거한다.
                    return true;
                }
                else
                {
        ...
        public bool RemoveMCC()
        {
            try
            {
                pHashtableMCC.Clear();        //HashTable에서 모든 Recipe를 제거한다.
                return true;
            }
            catch
            {
        ...
        public int MCCCount
        {
            get { return pHashtableMCC.Count; }
        }
